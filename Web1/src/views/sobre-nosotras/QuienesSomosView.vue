<template>
  <section id="quienes-somos" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'quienes-somos'</p>
    </div>
  </section>
</template>
...
import { fetchQuienesSomosPage } from '../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchQuienesSomosPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching quiénes somos:', err)
  } finally {
    loading.value = false
  }
})
