<template>
  <section id="nuestra-filosofia" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'nuestra-filosofia'</p>
    </div>
  </section>
</template>
...
import { fetchNuestraFilosofiaPage } from '../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchNuestraFilosofiaPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching nuestra filosofía:', err)
  } finally {
    loading.value = false
  }
})
