...
function makePage(overrides: Partial<WordPressPage> = {}): WordPressPage {
  return {
    ...
    status: 'publish',
    type: 'page',
    link: 'https://example.com/adult-anxiety',
    title: { rendered: 'Ansiedad (WP)' },
    content: {
      rendered:
...
describe('AnsiedadView', () => {
  ...
  it('renders intro, block titles, list items and text content after fetch resolves', async () => {
    vi.mocked(fetchAdultAnxietyPage).mockResolvedValue(makePage())
    ...
    expect(wrapper.text()).toContain('Ansiedad (WP)')
  ...
  it('falls back to the default title and does not crash when the page is null', async () => {
    vi.mocked(fetchAdultAnxietyPage).mockResolvedValue(null)
