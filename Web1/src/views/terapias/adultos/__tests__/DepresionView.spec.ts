...
function makeTherapy(overrides: Partial<TherapiePost> = {}): TherapiePost {
  return {
    ...
    status: 'publish',
    type: 'therapie',
    link: 'https://example.com/depresion-y-estado-de-animo',
    ...
    format: 'standard',
    acf: {
      therapy_name: 'Depresión y estado de ánimo (WP)',
      specialty: 'adult',
...
describe('DepresionView', () => {
  ...
  it('renders intro, block titles, list items and text content after fetch resolves', async () => {
    ...
    expect(fetchTherapieBySlug).toHaveBeenCalledWith('depresion-y-estado-de-animo')
