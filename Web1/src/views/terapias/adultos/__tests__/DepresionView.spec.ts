...
function makePage(overrides: Partial<WordPressPage> = {}): WordPressPage {
  return {
    ...
    slug: 'adult-depression',
    status: 'publish',
    type: 'page',
    link: 'https://example.com/adult-depression',
    title: { rendered: 'Depresión y estado de ánimo (WP)' },
    content: {
      rendered:
...
describe('DepresionView', () => {
  beforeEach(async () => {
    vi.clearAllMocks()
    router.push('/terapias/adultos/depresion')
    await router.isReady()
  })

  it('shows a loading indicator before the fetch resolves', () => {
    let resolvePromise: (value: WordPressPage | null) => void = () => {}
    vi.mocked(fetchAdultDepressionPage).mockReturnValue(
      new Promise((resolve) => {
        resolvePromise = resolve
      }),
    )

    const wrapper = mount(DepresionView, {
  ...
  it('renders intro, block titles, list items and text content after fetch resolves', async () => {
    vi.mocked(fetchAdultDepressionPage).mockResolvedValue(makePage())
    ...
    expect(wrapper.text()).toContain('Depresión y estado de ánimo (WP)')
  ...
  it('falls back to the default title and does not crash when the page is null', async () => {
    vi.mocked(fetchAdultDepressionPage).mockResolvedValue(null)
