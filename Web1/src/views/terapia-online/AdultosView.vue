<template>
  <section id="adultos" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'adultos'</p>
    </div>
  </section>
</template>
...
import { fetchAdultosPage } from '../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchAdultosPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching psicología adultos:', err)
  } finally {
    loading.value = false
  }
})
