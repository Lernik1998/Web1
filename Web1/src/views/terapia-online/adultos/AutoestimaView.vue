<template>
  <section id="autoestima" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'autoestima'</p>
    </div>
  </section>
</template>
...
import { fetchAutoestimaPage } from '../../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchAutoestimaPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching autoestima:', err)
  } finally {
    loading.value = false
  }
})
