<template>
  <section id="ansiedad" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'ansiedad'</p>
    </div>
  </section>
</template>
...
import { fetchAnsiedadPage } from '../../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchAnsiedadPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching ansiedad:', err)
  } finally {
    loading.value = false
  }
})
