<template>
  <section id="duelo" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'duelo'</p>
    </div>
  </section>
</template>
...
import { fetchDueloPage } from '../../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchDueloPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching duelo:', err)
  } finally {
    loading.value = false
  }
})
