<template>
  <section id="depresion" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'depresion'</p>
    </div>
  </section>
</template>
...
import { fetchDepresionPage } from '../../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchDepresionPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching depresión:', err)
  } finally {
    loading.value = false
  }
})
