<template>
  <section id="adolescentes" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'adolescentes'</p>
    </div>
  </section>
</template>
...
import { fetchAdolescentesPage } from '../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchAdolescentesPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching psicología adolescentes:', err)
  } finally {
    loading.value = false
  }
})
