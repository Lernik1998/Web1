<template>
  <section id="infantil" class="kb-page">
    ...
    <div v-else class="no-data">
      <p>No se encontró la página 'infantil'</p>
    </div>
  </section>
</template>
...
import { fetchInfantilPage } from '../../services/dataService'
...
onMounted(async () => {
  try {
    const response = await fetchInfantilPage()
    pageData.value = response
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Error desconocido'
    console.error('Error fetching psicología infantil:', err)
  } finally {
    loading.value = false
  }
})
